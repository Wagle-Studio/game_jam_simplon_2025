...
export class App {
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            const teasersSection = document.querySelector(".teasers");
            if (teasersSection) {
                const json = yield this.fetchData();
                if (json) {
                    const randomizedStudentWork = this.shuffleArray(json.data);
                    randomizedStudentWork.forEach((studentWork) => {
                        const teaserCard = this.createTeaserCard(studentWork);
